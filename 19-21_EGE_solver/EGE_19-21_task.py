...
frame_conditions = ttk.Frame(borderwidth=1, relief=GROOVE, padding=[8, 10])

header_main = ttk.Label(text="Задания 19-21 ЕГЭ", font=("Arial", 14))
header_main.pack(anchor=CENTER)

main_container = ttk.Frame(frame_conditions)
main_container.pack(fill=X, padx=6, pady=6)

left_frame = ttk.Frame(main_container)
left_frame.pack(side=LEFT, anchor=N, padx=(0, 20))

header_type = ttk.Label(left_frame, text="Выберите количество куч")
header_type.pack(anchor=W, padx=130)
...
def on_task_type_change(*args):
    ...
    if task_type.get() == task_2:
        if second_heap_label is None:
            second_heap_label = ttk.Label(left_frame, text="Введите начальное количество камней во 2-й куче")
            second_heap_label.pack(anchor=W, pady=(10, 2))
            
            second_heap_entry = Entry(left_frame, font=("Arial", 12), width=5)
            second_heap_entry.pack(anchor=CENTER, padx=6, pady=6)
    else:
...
task1_btn = ttk.Radiobutton(left_frame, text=task_1, value=task_1, variable=task_type)
task1_btn.pack(anchor=W, padx=170, pady=2)
  
task2_btn = ttk.Radiobutton(left_frame, text=task_2, value=task_2, variable=task_type)
task2_btn.pack(anchor=W, padx=170, pady=2)

header_win = ttk.Label(left_frame, text="Введите необходимое количество камней из условия")
header_win.pack(anchor=W, padx=50)

decor_win_rocks = {"master": left_frame, "font": ("Arial", 12), "width": 5}
win_rocks = Entry(**decor_win_rocks)
win_rocks.pack(anchor=CENTER, padx=6, pady=6)

header_limit = ttk.Label(left_frame, text="Введите максимальный размер кучи (S) или минимально возможный")
header_limit.pack(anchor=W, pady=(10, 0))

heap_limit_entry = Entry(left_frame, font=("Arial", 12), width=5)
heap_limit_entry.pack(anchor=CENTER, padx=6, pady=6)

right_frame = ttk.Frame(main_container)
right_frame.pack(side=LEFT, anchor=N)

header_oper = ttk.Label(right_frame, text="Введите возможные операции(например //2)")
header_oper.pack(anchor=W)

decor_entres_oper = {"master": right_frame, "font": ("Arial", 12), "width": 5}
oper1 = Entry(**decor_entres_oper)
oper1.pack(anchor=CENTER, padx=6, pady=6)
oper2 = Entry(**decor_entres_oper)
oper2.pack(anchor=CENTER, padx=6, pady=6)
oper3 = Entry(**decor_entres_oper)
oper3.pack(anchor=CENTER, padx=6, pady=6)

bad_move_var = BooleanVar()
bad_move_checkbox = ttk.Checkbutton(
    right_frame, 
...
frame_conditions.pack(fill=X)

solve_button = ttk.Button(root, text="Решить", command=lambda: show_answers())
...
def show_answers():
    ...
    answers_frame = ttk.Frame(borderwidth=1, relief=GROOVE, padding=[8, 10])
    answers_frame.pack(fill=X, padx=10, pady=10)
    
    answers_label = ttk.Label(answers_frame, text="Ответы:", font=("Arial", 12, "bold"))
    ...
    for i, text in enumerate(answers_texts):
        answer_label = ttk.Label(answers_frame, text=text, font=("Arial", 10))
...
def calculate_answers(task_19, task_20, task_21):
    if answers_frame is not None:
        answers = [
            f"Ответ 19: {task_19}",
            f"Ответ 20: {"".join(str(x) for x in task_20)}", 
            f"Ответ 21: {task_21}"
        ]
